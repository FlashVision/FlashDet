"""Lightweight backbone for FlashDet.

Independent implementation based on:
    Ma et al., "ShuffleNet V2: Practical Guidelines for Efficient CNN
...
def channel_mix(x: torch.Tensor, groups: int) -> torch.Tensor:
    """Channel mix (shuffle) operation."""
...
class LiteUnit(nn.Module):
    """Lightweight backbone basic unit — channel-split depthwise block."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return channel_mix(out, 2)


class LiteBackbone(nn.Module):
    """
    Lightweight backbone with channel mixing and depthwise separable convolutions.
    ...
    def __init__(
        ...
        for i, (repeat, out_ch) in enumerate(zip(repeats, channels[1:-1])):
            ...
            for j in range(repeat):
                ...
                stage.append(LiteUnit(in_ch, out_ch, stride, activation))
                in_ch = out_ch
    ...
    def _load_pretrained(self, model_size: str):
        ...
        try:
            ...
            logging.getLogger(__name__).info(
                "Loaded pretrained LiteBackbone %s", model_size
            )
        except Exception as e:
            import logging
            log = logging.getLogger(__name__)
            log.warning(
                "Could not load pretrained LiteBackbone %s weights: %s", model_size, e
            )
            log.warning(
